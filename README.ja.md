...
Webページのスクリーンショットをサッと撮って、かわいい吹き出しや矢印で注釈し、登録したアバター/アイコンをスタンプとしてワンクリ貼り付け。ローカル完結で、PNG保存もクリップボードコピーもすぐにできる拡張です。

### 特長
- **表示部分をキャプチャ**: 今見えている範囲をワンクリックで撮影
- **範囲選択でキャプチャ**: 必要なところだけドラッグで選んで切り取り
- **注釈ツール**: 選択 / 四角形 / 円形 / 矢印 / 吹き出し / スタンプ（画像）/ モザイク / 消しゴム
- **吹き出し**: 明/暗の配色切替、しっぽの向き切替、最寄りスタンプ方向へしっぽ自動吸着、確定後は自動で選択状態
- **スタンプ（画像）**: 透過PNG/WebP対応、左右/上下反転。プリセット保存→次回からワンクリ貼り付け
- **自動選択**: 作成直後にそのまま移動・リサイズできる選択状態に
- **条件付き有効化**: 対象に応じてツールバーのボタンが自動で有効/無効
- **スタイル設定**: カラーパレット、線の太さ（1〜20px）
- **出力**: PNGダウンロード / クリップボードコピー
- **ローカル完結**: 画像や注釈はローカル処理（外部送信なし）
